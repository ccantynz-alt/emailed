...
export const metadata: Metadata = {
  title: "Authentication — AlecRae API Docs",
  ...
  return (
    <div className="px-6 md:px-8 py-12 md:py-16 max-w-4xl mx-auto">
      <PageHeader
        ...
        description="The AlecRae API supports API keys, OAuth 2.0 bearer tokens, and JWT. All authenticated requests must use HTTPS."
      />
  ...
  -H "X-API-Key: vn_live_aBcDeFgHiJkLmNoPqRsTuVwXyZ012345"`}
        ...
        <div>
          ...
          <div className="space-y-3 text-blue-100/70 text-sm mb-4">
            <p>1. Redirect the user to <code className="text-cyan-300 font-mono text-xs">https://auth.alecrae.com/authorize</code> with your <code className="text-cyan-300 font-mono text-xs">client_id</code>, <code className="text-cyan-300 font-mono text-xs">redirect_uri</code>, <code className="text-cyan-300 font-mono text-xs">scope</code>, and <code className="text-cyan-300 font-mono text-xs">state</code>.</p>
            <p>2. After the user grants access, exchange the authorization code at <code className="text-cyan-300 font-mono text-xs">https://auth.alecrae.com/token</code>.</p>
          ...
          <CodeBlock
            code={`POST https://auth.alecrae.com/token
...
&client_secret=YOUR_CLIENT_SECRET`}
        ...
        <div>
          ...
          <p className="text-blue-100/70 mb-4 leading-relaxed">
            AlecRae issues standard JWTs signed with RS256. You can verify tokens using our JWKS endpoint:
          </p>
          <CodeBlock
            code={`GET https://auth.alecrae.com/.well-known/jwks.json`}
            language="http"
          />
          <CodeBlock
...
const JWKS = createRemoteJWKSet(
  new URL("https://auth.alecrae.com/.well-known/jwks.json")
);

const { payload } = await jwtVerify(token, JWKS, {
  issuer: "https://auth.alecrae.com",
  audience: "https://api.alecrae.com",
});`}
            language="javascript"
